...
if ($_SERVER["REQUEST_METHOD"] == "POST" && ($_REQUEST["save"] <> '' || $_REQUEST["apply"] <> '') && check_bitrix_sessid())
{
	...
	if ($strError == '')
	{
		...
		/* PERSONAL_BIRTHDAY - правльный формат даты */
		$arFields["PERSONAL_BIRTHDAY"] = date($DB->DateFormatToPHP(FORMAT_DATETIME), strtotime($arFields["PERSONAL_BIRTHDAY"]));




		// multiple photo
		// if(!$arFields["UF_PHOTOS"]["n0"])
		// {
		// 	$request = Application::getInstance()->getContext()->getRequest();
		// 	$arPhotos = $request->getFile("UF_PHOTOS");
		// 	$photo =array();
		// 	foreach ($arPhotos['type'] as $key => $type)
		// 	{
		// 		$photo['n' . $key] = array(
		// 				"name" => $arPhotos['name'][$key],
		// 				"size" => $arPhotos['size'][$key],
		// 				"tmp_name" => $arPhotos["tmp_name"][$key],
		// 				"type" => $arPhotos['type'][$key],
		// 				"del" => false
		// 		);
		// 	}

		// 	$arFields["UF_PHOTOS"] = $photo;
		// }

		if ($obUser->Update($arResult["ID"], $arFields))
		{
			// Добавить данные в ИНФОБЛОК
			$request = Application::getInstance()->getContext()->getRequest();
			$arPhotos = $request->getFile("PHOTOS");
			foreach ($arPhotos['type'] as $key => $type)
			{
				$photo = array(
					"name" => $arPhotos['name'][$key],
...
if (!empty($arParams["USER_PROPERTY"]))
{
	...
	if (count($arParams["USER_PROPERTY"]) > 0)
	{
		foreach ($arUserFields as $FIELD_NAME => $arUserField)
		{
			...
			if($FIELD_NAME == "UF_UF_TYPE_HOUSING")
			{
				$arrTypeHousing = BulldogUtils::getListItemHighloadblock(5); // HL Typehousing
				foreach($arrTypeHousing as $val)
				{
					$arResult["USER_PROPERTIES"]["DATA"][$FIELD_NAME]["FIELDS"][$val["ID"]] = $val["UF_NAME"];
				}
